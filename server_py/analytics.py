...
def stats_data(stream, ac):
        ...
        students.append(
            {
                ...
                "bestPlace": s.get("bestPlace"),
                "tabLeaves": s.get("tabLeaves"),
                "fastAnswers": s.get("fastAnswers") or 0,
                "suspicion": suspicion,
                "suspect": suspicion >= threshold,
    ...
    return {
        ...
        "stream": "all" if stream is None else stream,
        "backend": db.backend,
    }
...
def analytics_sheets(stream, ac):
    ...
    for t in d["tasks"]:
        tasks.append(
            [
                t["id"],
                t["title"],
                topic_name.get(t["topic"], t["topic"]),
                type_name.get(t["type"], t["type"]),
                t["level"],
                t["attempts"],
                t["correct"],
                t["accuracy"],
    ...
    for s in d["sessions"]:
        sessions.append(
            [
                s["id"],
                s["date"] or "",
                s["stream"] if s["stream"] is not None else "",
                s["status"],
                s["students"],
                s["answers"],
                s["correct"],
    ...
    header = (
        ...
        + [
            f"#{c['id']} {c['date'] or ''}"
            + (f" (п{c['stream']})" if c["stream"] is not None else "")
            for c in cols
        ]
        + ["Всего"]
    )
